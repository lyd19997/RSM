...
RequestList::RequestList(int topoVertexSize) {
	...
	for (int t = 0; t < (PEROID - 3 * MIN_DURATION); ++t)
	{
		...
		for (int j = 1; j <= arrivalRate; j++)
		{
			...
			push_back(Request(id++, src, (dst < src ? dst : dst + 1), t, t + rand() % (2 * MIN_DURATION) + MIN_DURATION, /*randomExponential(1.0)*MEAN_VALUE*/1, randomExponential(1.0)*MEAN_TRANSFER_SIZE));
		}
	}
